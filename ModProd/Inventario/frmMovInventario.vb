...
Public Class frmMovInventario
  ...
  Private strMaterial_Id As String
...
'End Sub
  ...
  Private Sub cmbProductos_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
      Handles cmbProducto.SelectedIndexChanged
        ...
        strMaterial_Id = selectedItem("Id").ToString
  ...
  Private Sub GrabarMovimiento()
    Dim obInventarios As New clInventarios()
    Dim detalles As New DataTable(), Ff As Integer

    detalles.Columns.Add("MaterialID", GetType(Integer))
    detalles.Columns.Add("Cantidad", GetType(Decimal))
    detalles.Columns.Add("PrecioUnitario", GetType(Decimal))

    ' Añadir detalles de los materiales al DataTable
    'detalles.Rows.Add(29606, 100, 15.5D)
    'detalles.Rows.Add(29609, 200, 5D)
    For Ff = 0 To grMovInventario.Rows.Count - 1
      detalles.Rows.Add(grMovInventario.Item("ColMaterial_Id", Ff).Value, grMovInventario.Item("ColCant", Ff).Value, CDec(grMovInventario.Item("ColCosto", Ff).Value))
    Next
    ...
    Dim resultado As Boolean = obInventarios.RegistrarMovimiento(My.Settings.cnnModProd, 1, 1, DateTime.Now, "Ingreso de nuevos materiales", "FACT001", detalles)
